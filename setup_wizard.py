...
from newsdigest.paths import is_windows, venv_python_path
...
def _venv_python() -> Path:
    """Return the venv Python executable for this OS."""
...
# Generated by setup wizard — re-run {_setup_command()} to change
SMTP_EMAIL={email}
...
def _prompt_schedule() -> str:
    ...
    if current_os == "macos":
        ...
        print(f"  {DIM}You can run the digest manually with: {_run_command()}{NC}")
        return ""
    ...
    while True:
        ...
        if answer in ("y", "yes"):
            ...
            if result.returncode == 0:
                ...
                print(f"    • Run {DIM}{_setup_command()}{NC} to redo the setup")
            break

        elif answer in ("n", "no"):
            print()
            info("No problem! You can always test later with:")
            print(f"    {DIM}{_run_command('--dry-run')}{NC}  (preview only)")
            print(f"    {DIM}{_run_command()}{NC}            (send for real)")
            break
        else:
            print(f"  {DIM}Please type y or n.{NC}")
...
def run_wizard() -> None:
    ...
    print()
    print(f"  {DIM}You can re-run this wizard anytime with: {_setup_command()}{NC}")
    print()
    ...
    if ENV_FILE.exists():
        ...
        while True:
            ...
            if answer in ("y", "yes"):
                ...
                print()
                info("Commands:")
                print(f"    {DIM}{_run_command('--dry-run')}{NC}  — Preview digest")
                print(f"    {DIM}{_run_command()}{NC}            — Send digest")
                print(f"    {DIM}{_setup_command()}{NC}                          — Re-run setup")
                print()
                return
            else:
                print(f"  {DIM}Please type y or n.{NC}")
...
if __name__ == "__main__":
    try:
        ...
        print()
        print()
        info(f"Setup cancelled. Run {_setup_command()} to try again.")
        print()
        sys.exit(0)
